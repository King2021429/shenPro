...
func (d *Dao) AIChatCommon(query string, history *[]model.Message, modelName string, temperature float64) string {
	// 将新的用户消息添加到历史记录中
	*history = append(*history, model.Message{
		Role:    "user",
		Content: query,
	...
	if modelName == model.MoonshotModel {
		reqBody = model.ChatRequest{
			Model:       modelName,
			Messages:    *history,
			Temperature: temperature,
		}
	} else if modelName == model.DeepSeekModel {
		prompt := ""
		for _, msg := range *history {
			if msg.Role == "user" {
				prompt += fmt.Sprintf("用户: %s\n", msg.Content)
		...
		fmt.Println("不支持的模型名称:", modelName)
		return ""
	}
	...
	client := &http.Client{}
	url := d.c.Moonshot.Url
	if modelName == model.DeepSeekModel {
		url = "http://localhost:11434/api/generate"
	}
	...
	} else if model.DeepSeekModel == modelName {
		respData = &model.OllamaResponse{}
	}
	...
	var result string
	if modelName == model.MoonshotModel {
		result = respData.(*struct {
			Choices []struct {
				Message model.Message `json:"message"`
			} `json:"choices"`
		}).Choices[0].Message.Content
	} else if modelName == model.DeepSeekModel {
		result = respData.(*model.OllamaResponse).Response
	}
